...
def model_fn(features, labels, mode, params):
    ...
    logits_concat = []
    for i in range(18):
        subnet_dropout = tf.layers.dropout(inputs=dense, rate=0.4, training=(mode == tf.estimator.ModeKeys.TRAIN))
        single_neuron = tf.tile(tf.constant(params['single_neuron'][i], shape=(1, 1), dtype=tf.float32),
                                multiples=(batch_size, 1))
        subnet_concat = tf.concat([subnet_dropout, single_neuron], axis=-1)
        subnet_dense = tf.layers.dense(inputs=subnet_concat, units=128, activation=tf.nn.relu)
        subnet_dropout = tf.layers.dropout(inputs=subnet_dense, rate=0.4, training=(mode == tf.estimator.ModeKeys.TRAIN))
        subnet_logits = tf.layers.dense(inputs=subnet_dropout, units=1)
